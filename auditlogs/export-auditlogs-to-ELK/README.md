...
## Процесс обновления контента
Рекомендуется подписаться на обновления в данном репозитории для оповещения о наличии нового обновления.
...
Для обновления необходимо обеспечить удаление текущией версии образа, скачивание новой версии и пересоздание контейнера:
...
Для обновления уже существующих элементов Kibana (saved queries, dashboards, detection rules), необходимо:
- Удалить существующий элемент или элементы через веб-интерфейс (Stack Management > Kibana > Saved Objects)
- Удалить скрытый системный индекс .state-audit-trails-index

При следующем запуске контейнера будут заново импортированы все отсутствующие элементы.
