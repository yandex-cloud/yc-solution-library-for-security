...
resource "yandex_kubernetes_cluster" "regional_cluster" {
  ...
  service_account_id      = data.terraform_remote_state.sa.outputs.prod_sa["sa-cluster"].id
  node_service_account_id = data.terraform_remote_state.sa.outputs.prod_sa["sa-nodes"].id
